...
@pytest.fixture()
def metadata(request) -> dict[str, Any]:
...
def test_checking_dmp_error(metadata: dict[str, Any]) -> None:
    '''
    dmpの形式抽出エラー
    - valueの値が規定値以外
    - keyがない
...
def test_json_validation_error(metadata: dict[str, Any]) -> None:
    '''
...
def test_errorcode(filepath: str) -> None:
    '''
