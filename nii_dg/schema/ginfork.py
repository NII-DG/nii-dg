...
class GinMonitoring(ContextualEntity):
    ...
    def validate(self, crate: "ROCrate") -> None:
        ...
        targets = [
            ...
            if ent["experimentPackageFlag"] is True
        ]
        sum_size = sum_file_size(self["contentSize"][-2:], targets)
        if sum_size > int(self["contentSize"][:-2]):
            error.add(
                "contentSize",
                "The total file size of ginfork.File labeled as an experimental package is larger than the defined size.",
            )
        ...
        if self["datasetStructure"] == "for_parameters":
            if "parameterExperimentList" not in self:
                ...
                param_dirs = [
                    Path(param_dir).joinpath(required_dir_name)
...
class File(BaseFile):
    ...
    def check_props(self) -> None:
        ...
        if is_absolute_path(self.id):
            error.add(
                "@id",
                "The value MUST be URL or relative path to the file, not absolute path.",
            )
