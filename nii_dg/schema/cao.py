...
class DMP(ContextualEntity):
    ...
    def check_props(self) -> None:
        ...
        if "dataNumber" in self:
            if self.id != f"#dmp:{self['dataNumber']}":
                error.add(
                    "@id",
                    "The value MUST be started with '#dmp:'and then the value of dataNumber property MUST come after it.",
                )
    ...
    def validate(self, crate: "ROCrate") -> None:
        ...
        if len(dmp_metadata_ents) == 0:
            ...
            if self["accessRights"] == "open access" and "distribution" not in [
                *self.keys(),
