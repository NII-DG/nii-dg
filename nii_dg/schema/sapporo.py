...
class File(BaseFile):
    ...
    def check_props(self) -> None:
        ...
        if is_absolute_path(self.id):
            error.add(
                "@id",
                "The @id value MUST be URL or relative path to the file, not absolute path.",
            )
    ...
    def validate(self, crate: "ROCrate") -> None:
        ...
        if len(sapporo_run_ents) == 0:
            error.add(
                "AnotherEntity",
                "Entity `SapporoRun` MUST be required with sapporo.File entity.",
            )
...
class Dataset(BaseDataset):
    ...
    def check_props(self) -> None:
        ...
        if not is_relative_path(self.id):
            error.add(
                "@id",
                "The value MUST be relative path to the directory, neither absolute path nor URL.",
            )
    ...
    def validate(self, crate: "ROCrate") -> None:
        ...
        if isinstance(self["outputs"], dict):
            ...
            if len(outputs) > 0:
                ...
                error.add(
                    "outputs",
                    f"Dataset entity with @id {self['outputs']['@id']} is required in this crate, but not found.",
                )
                raise error
        ...
        for ent in self["outputs"]["hasPart"]:
            if isinstance(ent, dict):
                ...
                if len(file) > 0:
                    ...
                    error.add(
                        "outputs",
                        f"File entity with @id {ent['@id']} is required in this crate, but not found.",
                    )
                    raise error
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for file_name in file_names:
                ...
                with urlopen(
                    f"{endpoint.rstrip('/')}/runs/{run_id}/data/outputs/{file_name}"
                ) as response:
                    with open(file_path, "wb") as f:
                        f.write(response.read())
                prev_file_entities = [
                    ent for ent in outputs_entities if file_name == ent["name"]
                ]
                if len(prev_file_entities) == 0:
                    error.add(
                        "outputs",
                        f"The file {file_name} is included in the result of re-execution, but this crate does not have File entity with @id {file_name}.",
                    )
                    raise error
                ...
                if "contentSize" in prev_file_ent:
                    ...
                    if content_size != prev_file_ent["contentSize"]:
                        error.add(
                            "outputs",
                            "The file size of {file_name}, {content_size}, does not match the `contentSize` value in {prev_file_ent}.",
                        )
                if "sha256" in prev_file_ent:
                    ...
                    if sha256 != prev_file_ent["sha256"]:
                        error.add(
                            "outputs",
                            "The hash of {file_name} does not match the `sha256` value in {prev_file_ent}.",
                        )
