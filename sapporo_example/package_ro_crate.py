...
def package_ro_crate(sapporo_endpoint: str, wf_results_dir: Path) -> None:
    ...
    for file_path in wf_results_dir.joinpath("outputs").glob("**/*"):
        file_ins = File(str(file_path.relative_to(wf_results_dir)), {"name": file_path.name})
...
def main() -> None:
    ...
    parser.add_argument("sapporo_endpoint", help="The endpoint of Sapporo, which is as seen from NII-DG server (default: http://sapporo-service:1122)",
                        default="http://sapporo-service:1122", nargs="?")
    parser.add_argument("wf_results_dir", help="The directory where the results of the workflow are stored (default: ./results)",
