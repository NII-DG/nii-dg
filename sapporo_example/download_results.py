...
def download_results(endpoint: str, run_id: str, output_dir: Path) -> None:
    ...
    for output_file in output_file_list:
        with urlopen(
            f"{endpoint.rstrip('/')}/runs/{run_id}/data/outputs/{output_file}"
        ) as response:
    ...
    with urlopen(
        f"{endpoint.rstrip('/')}/runs/{run_id}/data/run_request.json"
    ) as response:
...
def main() -> None:
    ...
    parser.add_argument(
        "output_dir",
        help="The output directory to save the results (default: ./results)",
        default="./results",
        nargs="?",
    )
